...
// «Осиротевшие» CSI-остатки без ведущего ESC: буфер вывода режется по размеру и
// может отрезать ESC, оставляя, например, `[?6n` или `[38;5;200m` обычным текстом.
// Требуем после `[` приватный маркер (?,>,=,!) или цифру и финальную букву —
// так не заденем обычный текст вроде `[section]`, `[TODO]`, `[1]`.
const ORPHAN_CSI_RE = new RegExp('\\[[?>=!\\d][\\d;]*[A-Za-z]', 'g')
